...
class Login extends CI_Controller {
        ...
        public function noticeview($id = NULL){        
            ...
            $session_started= $this->session->userdata('session_started');   
            $data['notices']     = $this->notice_model->get_notices(); 
            $data['notice_item'] = $this->notice_model->get_notice_by_id($id);  
            
            $this->load->view('templates/header');
            $this->load->view('login/publicnotice',$data);
            $this->load->view('templates/footer');
        ...
        public function verifyuser(){
           // echo "Invalid attempt".$invalid_attempt = $this->session->userdata('invalid_attempt'); 
            ...
            $data['firstImage'] = $firstImage;
            $this->form_validation->set_rules('username', 'Username', 'trim|required|min_length[8]|max_length[16]',array('required' => 'You must provide a valid %s.'));
            $this->form_validation->set_rules('password', 'Password', 'trim|required|min_length[8]|max_length[16]|callback_valid_pass');
            ...
            $login_attempt = $this->session->userdata('login_attempts'); 
                ...
                if($lock_unlock){// unlocked
                                    ...
                                    if(!(password_verify($password, $userpass))){//invalid pass   
                                     //   echo "attempt:".$login_attempt = $login_attempt+1;
                                        ...
                                        $this->session->unset_userdata('login_attempts');
                                        $this->session->set_userdata('login_attempts',$login_attempt);  
                                        if($login_attempt>3){
