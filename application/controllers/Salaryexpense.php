...
class Salaryexpense extends CI_Controller {
    ...
    public function index()
    {
        ...
        $data['all_salaryexpenses']  = $this->salaryexpense_model->get_all_salaryexpenses();    
        var_dump($data['all_salaryexpenses']);  
        $data['person']  = $this->person_model->get_person();
        $data['expense_types'] = $this->salaryexpense_model->get_sal_expensetypes();
      ...
      //  var_dump( $data['paid_person'] );
       ...
       // $data['branches'] = $this->branch_model->get_branches();
        $data['title'] = 'Salary payment information'; 
        $this->load->view('templates/header');
        $this->load->view('templates/menu');
        $this->load->view('salaryexpense/index', $data);
    ...
    public function create()
    {
      ...
      //  var_dump( $data['paid_person'] );
        ...
        if (($this->form_validation->run() === FALSE)||($error)|| empty($_FILES['memo_image']['name']))
        {
            $this->load->view('templates/header');
            $this->load->view('templates/menu');
            $this->load->view('salaryexpense/create', $data);
            $this->load->view('templates/footer'); 
        }
    
        else
        {
            move_uploaded_file($tmp, 'upload/salaryexpense/'.$image_name);
            $this->salaryexpense_model->set_salexpense($id=0,$image_name);
            $this->session->set_flashdata('confirmation',"Salary expense added successfully");
            redirect('salaryexpense/index/');
        }
    }
