...
class Supplier_model extends CI_Model {
    ...
    public function get_suppliers($id = FALSE)
    {
        if ($id === FALSE)
        {
            $query = $this->db->get('supplier_info');
            return $query->result_array();
        }
 
        $query = $this->db->get_where('supplier_info', array('id' => $id));
...
//print_r($myArray); 
    ...
    public function set_employee($id=0,$image_name=NULL,$poa_file_name=NULL)
    {
        ...
        if($this->input->post('dob_time')==NULL){
            $dob_time=NULL;
        }else $dob_time = date('Y-m-d', strtotime($this->input->post('dob_time'))); 
        
        if($this->input->post('dor_time')==NULL){
            $dor_time=NULL;
        }else $dor_time = date('Y-m-d', strtotime($this->input->post('dor_time'))); 
        
        if($this->input->post('dod_time')==NULL){
            $dod_time=NULL;
        }else $dod_time = date('Y-m-d', strtotime($this->input->post('dod_time')));
